...
function renderPage() {
  ...
  queryClient.setQueryData(["admin", "assessments"], [
    {
      id: "assessment-1",
  ...
  queryClient.setQueryData(["admin", "academic-years"], [{ id: "year-1", name: "2026-27", startDate: "2026-06-01", endDate: "2027-05-31", status: "CURRENT" }]);
  queryClient.setQueryData(["admin", "standards"], [
    { id: "standard-3", code: "STD_3", name: "Standard 3", displayOrder: 3 },
    { id: "standard-4", code: "STD_4", name: "Standard 4", displayOrder: 4 },
  ]);
  queryClient.setQueryData(["admin", "assessment-types"], [{ id: "type-1", code: "UNIT_TEST", displayName: "Unit Test", sortOrder: 1 }]);
  queryClient.setQueryData(["admin", "subjects"], []);

  render(
    <QueryClientProvider client={queryClient}>
      <MemoryRouter><AdminAssessmentsPage /></MemoryRouter>
    </QueryClientProvider>,
  );
}
...
describe("AdminAssessmentsPage", () => {
  it("filters assessments by standard and accurately explains the blocked result workflow", () => {
    ...
    fireEvent.change(screen.getByLabelText("Filter by standard"), { target: { value: "standard-3" } });
